    ...
    void drawHealthBar(SDL_Renderer* renderer, int x, int y, int w, int h, int health, int maxHealth) {
        ...
        Button settingButton = {{220, 250, 200, 60}, {0, 0, 255, 255}, "Reglage", 1};
        Button quitButton = {{220, 350, 200, 60}, {200, 0, 0, 255}, "Quit", 1};
        Button backButton = {{300, 500, 200, 60}, {200, 200, 200, 255}, "Retour", 1};
        ...
        SDL_Surface* settingSurface = TTF_RenderText_Blended(font, settingButton.label, color);
        SDL_Texture* settingText = SDL_CreateTextureFromSurface(renderer, settingSurface);
        ...
        SDL_Surface* backSurface = TTF_RenderText_Blended(font, backButton.label, color);
        ...
        while (running) {
            ...
            while (SDL_PollEvent(&e)) {
                ...
                if (e.type == SDL_QUIT)
                    ...
                    if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_h) {
                        player.health -= 10; // Enlève 10 PV quand on appuie sur H
                ...
                if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                    ...
                    else if (inMenu && isMouseInside(settingButton.rect, mx, my)) {
                        printf("Entrée dans les réglages\n");
                        inMenu = 0;
                        inSettings = 1;
            ...
            if (settingButton.visible) {
                if (isMouseInside(settingButton.rect, mx, my))
                    SDL_SetRenderDrawColor(renderer, 50, 50, 255, 255);
                else
                    SDL_SetRenderDrawColor(renderer, 0, 0, 200, 255);

                SDL_RenderFillRect(renderer, &settingButton.rect);

                SDL_Rect settingRectText = {0, 0, 0, 0};
                SDL_QueryTexture(settingText, NULL, NULL, &settingRectText.w, &settingRectText.h);
                settingRectText.x = settingButton.rect.x + (settingButton.rect.w - settingRectText.w) / 2;
                settingRectText.y = settingButton.rect.y + (settingButton.rect.h - settingRectText.h) / 2;
                SDL_RenderCopy(renderer, settingText, NULL, &settingRectText);
            }
